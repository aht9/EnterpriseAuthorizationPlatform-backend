...
public static class AuthEndpoints
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var result = await handler.HandleAsync(new RegisterCommand(request.Email, request.Password, context), cancellationToken);
        await auditSink.RecordAsync("IdentityService.user_registered", context.TenantId, context.CorrelationId, result.Value?.UserId, result.IsSuccess, result.IsFailure ? result.Error.Description : null, cancellationToken);

        return result.IsSuccess
            ? TypedResults.Created($"/api/v1/identity/users/{result.Value!.UserId}", ApiResponse<RegisterResponse>.Ok(result.Value!, context.CorrelationId))
    ...
    {
        var context = EndpointContext.From(http);
        var result = await handler.HandleAsync(new LoginCommand(request.Email, request.Password, request.MfaCode, context), cancellationToken);
    ...
    {
        var context = EndpointContext.From(http);
        var result = await handler.HandleAsync(new RefreshTokenCommand(request.RefreshToken, context), cancellationToken);
