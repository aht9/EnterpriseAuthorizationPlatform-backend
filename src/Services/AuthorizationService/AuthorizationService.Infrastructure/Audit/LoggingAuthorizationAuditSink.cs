...
public sealed class LoggingAuthorizationAuditSink(ILogger<LoggingAuthorizationAuditSink> logger) : IAuthorizationAuditSink
{
    ...
    {
        logger.LogInformation("Authorization decision tenant={TenantId} subject={SubjectId} action={Action} resource={ResourceType}/{ResourceId} allowed={Allowed} reason={ReasonCode} correlation={CorrelationId}", input.TenantId, input.SubjectId, input.Action.Value, input.Resource.ResourceType, input.Resource.ResourceId, result.IsAllowed, result.ReasonCode, result.CorrelationId);
